...
* 容错性。主要是CAP理论中的最终一致性的体现。
* 复杂性隔离。通过批处理层与流式处理层之间相互分离，不会把每个层的复杂性蔓延到其他层进行耦合，可以很好的提高整个系统的鲁棒性和可靠性。

### lambda架构缺点
* 开发和维护的复杂性问题。Lambda 架构需要在两个不同的 API 中对同样的业务逻辑进行两次编程：一次为批量计算的ETL系统，一次为流式计算的Streaming系统。维护的复杂性大大提高。
